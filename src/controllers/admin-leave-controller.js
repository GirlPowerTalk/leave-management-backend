...
adminLeaveRouter.put("/update-application/:id", async (req, res) => {
   try {
      ...
      body.leaves.forEach(leave => {
         const addOrMergeLeave = (leaveTypeId, leaveCount) => {
            if (leaveTypeId !== null) {
               const existingLeave = transformedLeaves.find(l => l.leaveTypeId === leaveTypeId);
               if (existingLeave) {
                  existingLeave.leaveCount += leaveCount;
               } else {
                  transformedLeaves.push({ leaveTypeId, leaveCount });
               }
            }
         };
            ...
            await Promise.all(
               transformedLeaves?.map(leave =>
                  prisma.userLeave.update({
                     ...
                     data: {
                        totalLeaves: {
                           decrement: +leave.leaveCount
                        }
                     }
                  })
               ),
               applicationDetails?.userLeave?.map(leave =>
                  prisma.userLeave.update({
                     where: {
                        userId_leaveTypeId: {
                           userId: +applicationDetails?.userId,
